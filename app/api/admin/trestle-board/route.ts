...
// POST - Create a new trestle board
export async function POST(request: NextRequest) {
  return withAdminAuth(request, async (authenticatedReq: AuthenticatedRequest) => {
    try {
      ...
      if (existingBoard) {
        return NextResponse.json(
          { error: 'A trestle board already exists for this month and year' },
          { status: 400 }
        );
      }
      ...
      // Create the trestle board with meals
      const board = await prisma.trestleBoard.create({
        data: {
          month: parseInt(month),
          year: parseInt(year),
          title,
      ...
      return NextResponse.json({
        data: board,
        message: 'Trestle board created successfully',
      });
    } catch (error) {
      console.error('Error creating trestle board:', error);
      return NextResponse.json(
        { error: 'Failed to create trestle board' },
        { status: 500 }
      );
    }
  });
